...
class HBasePreLoadedDataQuerySuite extends TestBaseWithNonSplitData {

  val TestTablePreLoadName = "TestTablePreLoad"
  val TestHBaseTablePreLoadName: String = s"Hb$TestTablePreLoadName"

  override protected def beforeAll() = {
//    super.beforeAll()

    val testTableCreationSQL =
      s"""CREATE TABLE $TestTablePreLoadName(
         |  strcol STRING,
         |  bytecol TINYINT,
         |  shortcol SMALLINT,
         |  intcol INTEGER,
         ...
         |OPTIONS(
         |  tableName "$TestTablePreLoadName",
...
//      put.add(Bytes.toBytes(TestHbaseColFamilies(0)), Bytes.toBytes("hbytecol"), Bytes.toBytes(line(1).toByte))
      ...
      put.add(Bytes.toBytes(TestHbaseColFamilies(1)), Bytes.toBytes("hintcol"), Bytes.toBytes(line(3).toInt))
      put.add(Bytes.toBytes(TestHbaseColFamilies(1)), Bytes.toBytes("hlongcol"), Bytes.toBytes(line(4).toLong))
      put.add(Bytes.toBytes(TestHbaseColFamilies(1)), Bytes.toBytes("hfloatcol"), Bytes.toBytes(line(5).toFloat))
      table.put(put)
    }
  ...
  override protected def afterAll() = {
    super.afterAll()
    runSql("DROP TABLE " + TestTablePreLoadName)
  }

  var testnm = "StarOperator * with limit"
  test("StarOperator * with limit") {
    val query1 =
      s"""SELECT * FROM $TestTablePreLoadName LIMIT 3"""
        .stripMargin
    ...
    val sql2 =
      s"""SELECT * FROM $TestTablePreLoadName LIMIT 2"""
        .stripMargin
  ...
  test("Select all cols with filter") {
    val query1 =
      s"""SELECT * FROM $TestTablePreLoadName WHERE shortcol < 12345 LIMIT 2"""
        .stripMargin
  ...
  test("Select all cols with order by") {
    val query1 =
      s"""SELECT * FROM $TestTablePreLoadName WHERE shortcol < 12344 ORDER BY strcol DESC LIMIT 2"""
        .stripMargin
  ...
  test("Select specific cols with filter") {
    val query1 =
      s"""SELECT doublecol AS double1, -1 * doublecol AS minusdouble,
         ...
         | bytecol, shortcol, intcol, longcol, floatcol FROM $TestTablePreLoadName WHERE strcol LIKE
  ...
  test("Mixed And/or predicates") {
    val query1 = s"""SELECT doublecol AS double1, -1 * doublecol AS minusdouble,
     substr(strcol, 2) AS substrcol, doublecol, strcol,
     bytecol, shortcol, intcol, longcol, floatcol FROM $TestTablePreLoadName
