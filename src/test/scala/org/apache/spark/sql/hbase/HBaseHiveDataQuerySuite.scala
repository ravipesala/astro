...
class HBaseHiveDataQuerySuite extends TestBaseWithNonSplitData {
  ...
  val TestHBaseTableHiveName = "TestTableHiveName"
  ...
  override protected def beforeAll() = {
    ...
    val testTableCreationHiveSQL =
      s"""CREATE EXTERNAL TABLE $TestHBaseTableHiveName(
         |  strcol STRING,
         |  bytecol TINYINT,
         |  shortcol SMALLINT,
         |  intcol INT,
  ...
  def createHiveTable(tableName: String, hbaseTable: String, creationSQL: String) = {
    ...
    TestHbase.runSqlInHive("SET hbase.zookeeper.property.clientPort=" + sparkContext.hadoopConfiguration.get("hbase.zookeeper.property.clientPort"))
    TestHbase.runSqlInHive("SET SET fs.trash.interval=30")
    if (TestHbase.catalog.tableExists(Seq(tableName))) {
      val dropSql = s"DROP TABLE $tableName"
      runSqlInHive(dropSql)
    }

    try {
      logInfo(s"invoking $creationSQL ..")
      runSqlInHive(creationSQL)
    } catch {
